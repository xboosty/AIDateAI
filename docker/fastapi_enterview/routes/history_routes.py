...
from ai.transcription import transcribe_speech_reconigtion
import os
...
def create_history(
    ...
    if is_response and audio is not None:        
        try:                        
            ...
            #message = transcribe_speech_reconigtion(audio).text_result
            #history_data['message'] = message
            history_data['message'] = 'path_audio:'+path_audio
        except Exception as e:
            raise HTTPException(status_code=500, detail="Audio file could not be saved")
        
    if history_data['message'] is None:
        raise HTTPException(status_code=400, detail="Message could not be resolved")
